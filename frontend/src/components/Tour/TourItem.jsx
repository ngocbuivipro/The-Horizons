...
// Icons
import { FaMapMarkerAlt, FaRegClock, FaUserFriends, FaStar, FaCheck } from "react-icons/fa";
    ...
    return (
        ...
        >
            ...
            `}>
                ...
                {/* Wishlist: Đưa về góc phải */}
                <div
                    className="absolute top-3 right-3 w-8 h-8 flex items-center justify-center bg-white/90 backdrop-blur-md rounded-full text-gray-500 hover:text-pink-500 hover:bg-white transition-all shadow-sm"
                    onClick={handleWishlist}
                >
                    {click ? <AiFillHeart size={16} className="text-pink-500" /> : <AiOutlineHeart size={16} />}
                </div>
            </div>
            ...
            <div className={`flex flex-1 flex-col justify-between ${isGrid ? 'p-5 pt-3' : 'py-1'}`}>
                ...
                <div className={`flex items-end w-full mt-4 ${isGrid ? 'justify-between border-t border-gray-50 pt-3' : 'justify-between'}`}>
                    ...
                    <div>
                        <span className="text-[11px] text-gray-400 font-medium block uppercase tracking-wider mb-0.5">Start form</span>
                    ...
                    {/* Nút xem cho Grid mode */}
                    {isGrid && (
                        <button className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-teal-600 hover:bg-teal-600 hover:text-white transition-all">
                            <FaCheck size={12}/>
                        </button>
                    )}
                </div>
