...
import { Button, Input, Select, TimePicker, Tooltip, Switch, Spin, Rate } from "antd";
...
const AdminViewEditHotel = () => {
  ...
  const handleAddCustomRoomType = () => {
    if (inputRoomType.trim()) {
      setRoomTypeDefault(prev => [...new Set([...prev, inputRoomType])]);
      setRoomType(prev => [...new Set([...prev, inputRoomType])]);
      setInputRoomType("");
    }
  };
              ...
              <div className="flex flex-col gap-4">
                <div className="flex gap-3 w-full md:max-w-md">
                  <input
                      ...
                      className="flex-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-700 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all"
                  />
                  <button
                      type="button"
                      onClick={handleAddCustomRoomType}
                      className="px-4 py-3 bg-gray-800 text-white rounded-xl hover:bg-gray-700 transition shadow-md"
                  >
                    Add Type
                  </button>
                </div>
