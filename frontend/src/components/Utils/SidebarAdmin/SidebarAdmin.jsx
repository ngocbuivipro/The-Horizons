...
// Import các Icons
import {FaBars, FaList, FaInfoCircle, FaMapMarkedAlt, FaChevronDown, FaBus, FaShip, FaCar} from "react-icons/fa";
...
const SidebarAdmin = () => {
    ...
    const isCarActive = [
        "/dashboard-view-car",
        "/dashboard-create-car-transfer",
        "/dashboard-create-route-transfer",
    ].includes(location.pathname);
    ...
    useEffect(() => {
        ...
        if (isCarActive) setIsCarOpen(true);
        if (isTourActive) setIsTourOpen(true);
        if (isBusActive) setIsBusOpen(true);
                        ...
                        <ul className="flex flex-col gap-1">
                            ...
                            <li className="flex flex-col gap-1">
                                <div
                                    ...
                                    // LOGIC FIX: Thêm isBusActive tương tự như Tour
                                    className={`flex items-center justify-between p-2 rounded-md cursor-pointer transition-all duration-300 px-2
                                      ${(isBusOpen || isBusActive) ? "bg-indigo-50 text-indigo-600" : "text-gray-600 hover:bg-gray-50"}
                                    `}
                                >
                                    <div className="flex items-center gap-3">
                                        <FaBus size={18} className={isBusActive ? "text-indigo-600" : "text-gray-400"}/>
                                        <span className="text-sm font-semibold">Bus</span>
                                    </div>
                                    <FaChevronDown
                                        className={`text-xs transition-transform duration-300 ${isBusOpen ? "rotate-180" : "rotate-0"}`}
                                    />
                                </div>

                                <div
                                    className={`overflow-hidden transition-all duration-300 ease-in-out
                                    ${isBusOpen ? "max-h-40 opacity-100 mt-1" : "max-h-0 opacity-0"}`
                                    }
                                >
                                    <ul className="flex flex-col gap-1">
                                        <Link to="/dashboard-view-bus" onClick={handleMenuClick}>
                                            <li className={childNavStyle("/dashboard-view-bus")}>All Buses</li>
                                        </Link>
                                        <Link to="/dashboard-add-bus" onClick={handleMenuClick}>
                                            <li className={childNavStyle("/dashboard-add-bus")}>Add New Bus</li>
                                        </Link>
                                    </ul>
                                </div>
                            </li>
                            ...
                            <li className="flex flex-col gap-1">
                                <div
                                    ...
                                    className={`flex items-center justify-between p-2 rounded-md cursor-pointer transition-all duration-300 px-2
                                      ${(isCarOpen || isCarActive) ? "bg-indigo-50 text-indigo-600" : "text-gray-600 hover:bg-gray-50"}
                                    `}
                                >
                                    <div className="flex items-center gap-3">
                                        {/* BUG: Biến isCruiseActive đang được sử dụng ở mục Car. Phải đổi thành isCarActive */}
                                        <FaCar size={18} className={isCarActive ? "text-indigo-600" : "text-gray-400"}/>
                                        <span className="text-sm font-semibold">Car</span>
                                    </div>
                                    <FaChevronDown
                                        className={`text-xs transition-transform duration-300 ${isCarOpen ? "rotate-180" : "rotate-0"}`}
                                    />
                                </div>

                                <div
                                    className={`overflow-hidden transition-all duration-300 ease-in-out
                                    ${isCarOpen ? "max-h-40 opacity-100 mt-1" : "max-h-0 opacity-0"}`
                                    }
                                >
                                    <ul className="flex flex-col gap-1">
                                        <Link to="/dashboard-view-car" onClick={handleMenuClick}>
                                            <li className={childNavStyle("/dashboard-view-car")}>All Cars</li>
                                        </Link>

                                        <Link to="/dashboard-create-car-transfer" onClick={handleMenuClick}>
                                            {/* CRITICAL BUG: Đường dẫn trong childNavStyle không khớp với Link to.
                                                Link là "...create-car..." nhưng style check "...add-car...".
                                                Style sẽ không bao giờ active.
                                            */}
                                            <li className={childNavStyle("/dashboard-create-car-transfer")}>Add New Car</li>
                                        </Link>

                                        <Link to="/dashboard-create-route-transfer" onClick={handleMenuClick}>
                                            {/* CRITICAL BUG: Tương tự như trên. Link "...create-route..." != style check "...add-route...". */}
                                            <li className={childNavStyle("/dashboard-create-route-transfer")}>Add New Route</li>
                                        </Link>
                                    </ul>
                                </div>
                            </li>
                            ...
                            <Link to="/dashboard-about" onClick={handleMenuClick}>
                                <li className={navStyle("/dashboard-about")}>
                                    <FaInfoCircle size={18}
                                                  className={location.pathname === "/dashboard-about" ? "text-indigo-600" : "text-gray-400"}/>
